...
export const ManageUnitsScreen = ({ route }: { route: { params: { propertyID: number } } }) => {
  ...
  const editApartments = useMutation(
    (obj: EditApartment[]) =>
      axios.patch(`${endpoints.updateApartments}${route.params.propertyID}`, obj),
    {
      onMutate: () => {
        setLoading(true);
      },
        ...
        <Formik
          ...
          {({
            ...
            const removeUnit = (index: number) => {
              const newApartments = values.apartments.filter((_, idx) => idx !== index);
              ...
              <View>
                {values.apartments?.map((apartment, index) => (
                  <ManageUnitsCard
                    apartment={apartment}
                    key={index}
                    removable={apartmentData && index > apartmentData.length - 1 ? true : false}
                    index={index}
                    errors={errors}
                    touched={touched}
