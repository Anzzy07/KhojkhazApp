  ...
  return useMutation(
    ...
    }) => createMessage(conversationID, senderID, receiverID, text, user?.accessToken),
    {
      onMutate: async ({ author, text, conversationID, receiverID, senderID }) => {
        ...
        const prevConversations: TransformedConversation[] | undefined = queryClient.getQueryData(
          queryKeys.conversations
        );
        const prevSelectedConversation: SelectedConversation | undefined = queryClient.getQueryData(
        ...
        if (prevSelectedConversation) {
          ...
          queryClient.setQueryData(queryKeys.selectedConversation, newSelectedConversation);
        }

        if (prevConversations) {
          const newConversations = [...prevConversations];
          const index = newConversations.findIndex((i) => i.ID === conversationID);
      ...
      onError: (err, vars, context) => {
        queryClient.setQueryData(queryKeys.conversations, context?.prevConversations);
        queryClient.setQueryData(queryKeys.selectedConversation, context?.prevSelectedConversation);
      },
      onSuccess: (_, { author, conversationID, receiverID, senderID, text }) => {
