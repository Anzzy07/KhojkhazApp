  ...
  return useMutation(
      ...
      saveOrUnsaveProperty(propertyID, op, user?.ID),
    {
      onMutate: async ({ propertyID, op }) => {
        ...
        const prevSavedProperties: Property[] | undefined = queryClient.getQueryData(
          queryKeys.savedProperties
        );
        const prevSearchedProperties: Property[] | undefined = queryClient.getQueryData(
          queryKeys.searchProperties
        );
        const prevSelectedProperty: Property | undefined = queryClient.getQueryData(
        ...
        if (prevSelectedProperty?.ID === propertyID) {
          ...
          queryClient.setQueryData(queryKeys.selectedProperty, newSelectedProperty);
        }

        if (op === 'remove') {
          if (prevSavedProperties) {
            const newSavedProperties = prevSavedProperties.filter((i) => i.ID !== propertyID);
            queryClient.setQueryData(queryKeys.savedProperties, newSavedProperties);
          }
        ...
        queryClient.setQueryData(queryKeys.searchProperties, prevSearchedProperties);
      ...
      onError: (err, vars, context) => {
        ...
        queryClient.setQueryData(queryKeys.selectedProperty, context?.prevSelectedProperty);
      },
      onSettled: () => {
