...
const AmenityCheckBox = ({
  ...
  return (
    <CheckBox checked={checked} style={styles.defaultMarginVertical} onChange={onPress}>
      {name}
    </CheckBox>
  );
};
...
const Amenities = ({
  ...
  return (
    <View>
      <ModalHeader xShown text="Property Amenities" onPress={cancel ? cancel : undefined} />
      ...
      {unitAmenities.map((i) => (
        <AmenityCheckBox
          name={i}
          key={i}
          onPress={amenitiesMap.get(i) ? () => handleDeleteAmenity(i) : () => handleAddAmenity(i)}
